    ...
    const ALL_MODULE_LABELS = {
      ...
      financeiro: 'Financeiro', receber: 'A Receber', relatorios: 'Relatórios',
      settings: 'Configurações'
    };
    const ALL_MODULE_ICONS = {
      dashboard: '🏠', pdv: '🛒', vendas: '📋', os: '🔧', produtos: '📦',
      clientes: '👥', financeiro: '💰', receber: '💳', relatorios: '📊',
      settings: '⚙️'
    };
  ...
  `, `<button class="btn" onclick="closeModal()">Cancelar</button>
    ...
    async function navigate(page) {
      ...
      const titles = {
        ...
        produtos: 'Produtos', clientes: 'Clientes', financeiro: 'Financeiro', receber: 'A Receber',
      ...
      const renderer = {
        ...
        produtos: renderProdutos, clientes: renderClientes, financeiro: renderFinanceiro, receber: renderFinanceiro,
  ...
  `;
    ...
    function buildPermGrid(papel, permissoesSelecionadas = []) {
      ...
      // Exibe todos os módulos do sistema
      const planoModulos = Object.keys(ALL_MODULE_LABELS);
      return `
    <div class="section-title" style="margin-top:4px">Permissões por Módulo</div>
    ${isAdmin ? `<div style="font-size:12px;color:var(--green);margin-bottom:8px">✅ Administrador tem acesso total a todos os módulos automaticamente.</div>` : ''}
      ...
      ${planoModulos.map(m => {
        ...
        return `
          <div class="perm-item ${checked ? 'checked' : ''} ${isAdmin ? 'admin-locked' : ''}"
               onclick="${isAdmin ? '' : `togglePerm(this,'${m}')`}" data-module="${m}">
            <div class="perm-check">${checked ? '✓' : ''}</div>
            <span class="perm-label">${ALL_MODULE_ICONS[m] || '•'} ${ALL_MODULE_LABELS[m] || m}</span>
          </div>`;
      }).join('')}
    </div>`;
    }
  ...
  <div id="pdv-wrap">
    ...
    <div id="pdv-sidebar">
      ...
      <div id="cart-total">
        ...
        <div class="total-row"><span>Desconto (R$)</span><input type="number" id="cart-desconto" value="0" min="0" step="0.01" onchange="updateTotals()" style="width:80px;text-align:right;padding:3px 6px"></div>
    ...
    function addToCart(id) {
      ...
      if (ex) { ex.quantidade++; ex.subtotal = ex.quantidade * ex.preco_unitario; }
      else { cart.push({ produto_id: id, produto_nome: p.nome, quantidade: 1, preco_unitario: p.preco_venda, subtotal: p.preco_venda, desconto: 0 }); }
      renderCart();
    }
    function removeFromCart(id) { cart = cart.filter(x => x.produto_id !== id); renderCart(); }
    function updateQty(id, delta) { const it = cart.find(x => x.produto_id === id); if (!it) return; it.quantidade = Math.max(0.01, (parseFloat(it.quantidade) || 1) + delta); it.subtotal = it.quantidade * it.preco_unitario; renderCart(); }
    function setQty(id, val) { const it = cart.find(x => x.produto_id === id); if (!it) return; it.quantidade = Math.max(0.01, parseFloat(val) || 1); it.subtotal = it.quantidade * it.preco_unitario; updateTotals(); }
    ...
    function updateTotals() {
      ...
      const desc = parseFloat(document.getElementById('cart-desconto')?.value || 0) || 0;
  ...
  `, `<button class="btn" onclick="closeModal()">Cancelar</button>
    ...
    async function finalizarVenda() {
      ...
      if (r?.ok) {
        ...
        renderCart();
        renderProdutosGrid();
        document.getElementById('cart-desconto').value = 0;
  ...
  <td style="display:flex;gap:4px"><button class="btn btn-sm" onclick="verOS(${o.id})">Ver</button><button class="btn btn-sm" onclick="editarOS(${o.id})">Editar</button></td></tr>`).join('')}</tbody></table>`;
    ...
    function buildOSForm(o) {
      ...
      const vsVal = o ? fmt(o.valor_servico) : 'R$ 0,00';
      const descVal = o ? fmt(o.desconto) : 'R$ 0,00';
      const ck = JSON.parse(o?.checklist || '{}');
      const items = ['Aparelho Liga', 'Tela Quebrada', 'Conector Carga', 'Bateria Carga', 'Câmeras', 'Microfone', 'Auto Falante', 'Botões', 'Carcaça Boa', 'Gaveta de Chip'];
    ...
    function switchOSTab(idx) {
      ...
      document.getElementById(`os-tab-${idx}`).classList.add('active');
      document.querySelectorAll('.tab-btn')[idx].classList.add('active');
    }

    function onOSClientChange(id) {
      if (!id) {
        document.getElementById('os-nc-nome').value = '';
        document.getElementById('os-nc-doc').value = '';
        document.getElementById('os-nc-tel').value = '';
        return;
      }
      const cli = allClientes.find(c => c.id == id);
      if (cli) {
        document.getElementById('os-nc-nome').value = cli.nome || '';
        document.getElementById('os-nc-doc').value = cli.cpf_cnpj || '';
        document.getElementById('os-nc-tel').value = cli.telefone || '';
      }
    }
    ...
    function togglePatternPoint(idx) {
      const inp = document.getElementById('os-spad');
      let val = inp.value || '';
      const parts = val.split('').map(Number).filter(n => n > 0);
      const existingIdx = parts.indexOf(idx);

      if (existingIdx !== -1) {
        // Remove if already exists? Actually for patterns usually you click to add.
        // Let's make it toggle: if clicked again, it removes itself and everything AFTER it.
        parts.splice(existingIdx);
      } else {
        parts.push(idx);
      }
    ...
    function renderPattern() {
      ...
      dots.forEach(d => {
        ...
        d.removeAttribute('data-seq');
      });
      parts.forEach((p, i) => {
        const dot = document.querySelector(`.p-dot[data-idx="${p}"]`);
        if (dot) {
          dot.classList.add('active');
          dot.setAttribute('data-seq', i + 1);
        }
      });
    }

    function calcOsTotal() {
      const vsStr = document.getElementById('os-vs')?.value || '0';
      const descStr = document.getElementById('os-desc')?.value || '0';
      const vs = parseMoney(vsStr);
      const d = parseMoney(descStr);
      const t = document.getElementById('os-tot');
      if (t) t.value = fmt(Math.max(0, vs - d));
    }

    function getOSData() {
      calcOsTotal();
      const cliId = document.getElementById('os-cli')?.value || null;
      const nome = document.getElementById('os-nc-nome')?.value || '';

      if (!nome) { notify('Nome do cliente é obrigatório', 'error'); throw 'Nome obrigatório'; }

      let novoCli = null;
      if (!cliId) {
        novoCli = {
          nome: nome,
          cpf_cnpj: document.getElementById('os-nc-doc')?.value || '',
          telefone: document.getElementById('os-nc-tel')?.value || ''
        };
      }
      ...
      const ck = {};
      document.querySelectorAll('.os-ck').forEach(el => { ck[el.dataset.item] = el.checked; });
      ...
      return {
        ...
        novo_cliente: novoCli || undefined,
        ...
        status: document.getElementById('os-st')?.value || 'ABERTA',
        prioridade: document.getElementById('os-pri')?.value || 'NORMAL',
        previsao: document.getElementById('os-prev')?.value || null,
        valor_servico: vs,
        valor_pecas: 0,
        desconto: desc,
        total: Math.max(0, vs - desc),
        forma_pagamento: document.getElementById('os-pgto')?.value || 'DINHEIRO',
    ...
    async function salvarOS() { const d = getOSData(); const r = await api('/api/os', 'POST', d); if (r?.ok) { notify(`OS ${r.numero} criada!`, 'success'); closeModal(); loadOS(); } else notify('Erro', 'error'); }
    async function atualizarOS(id) { const d = getOSData(); const r = await api(`/api/os/${id}`, 'PUT', d); if (r?.ok) { notify('OS atualizada!', 'success'); closeModal(); loadOS(); } else notify('Erro', 'error'); }
    ...
    async function renderProdutos() {
      ...
      document.getElementById('topbar-actions').innerHTML = `<button class="topbar-btn primary" onclick="novoProduto()">+ Novo Produto</button>`;
      const catOpts = allCategorias.map(c => `<option value="${c.id}">${c.nome}</option>`).join('');
      document.getElementById('content').innerHTML = `
    <div class="filters">
      <div class="filter-group"><label>Buscar</label><input type="text" id="pf-q" placeholder="Nome ou código..."></div>
      <div class="filter-group"><label>Categoria</label><select id="pf-cat"><option value="">Todas</option>${catOpts}</select></div>
      <div class="filter-group" style="justify-content:flex-end;padding-top:8px"><label style="display:flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="pf-baixo"> Estoque Baixo</label></div>
    ...
    async function loadProdutos() {
      const q = document.getElementById('pf-q')?.value || ''; const cat = document.getElementById('pf-cat')?.value || ''; const baixo = document.getElementById('pf-baixo')?.checked ? '1' : '';
      const data = await api(`/api/produtos?q=${encodeURIComponent(q)}&categoria=${cat}&estoque_baixo=${baixo}`); if (!data) return;
      ...
      tbl.innerHTML = `<table><thead><tr><th>Código</th><th>Nome</th><th>Categoria</th><th>Custo</th><th>Venda</th><th>Margem</th><th>Estoque</th><th>Un</th><th></th></tr></thead>
  <tbody>${data.map(p => { const margem = p.preco_custo > 0 ? ((p.preco_venda - p.preco_custo) / p.preco_custo * 100).toFixed(1) : '-'; const eb = p.estoque <= p.estoque_minimo; return `<tr><td class="mono">${p.codigo || '-'}</td><td><b>${p.nome}</b></td><td>${p.categoria_nome || '-'}</td><td class="mono">${fmt(p.preco_custo)}</td><td class="mono fw7">${fmt(p.preco_venda)}</td><td class="mono ${parseFloat(margem) > 0 ? 'text-green' : 'text-red'}">${margem}%</td><td><span class="badge ${eb ? 'badge-red' : 'badge-green'}">${p.estoque}</span></td><td>${p.unidade}</td><td style="display:flex;gap:4px"><button class="btn btn-sm" onclick="editarProduto(${p.id})">Editar</button><button class="btn btn-sm btn-danger" onclick="deletarProduto(${p.id})">Del</button></td></tr>`; }).join('')}</tbody></table>`;
    }

    function buildProdutoForm(p) {
      const catOpts = allCategorias.map(c => `<option value="${c.id}" ${p && p.categoria_id == c.id ? 'selected' : ''}>${c.nome}</option>`).join('');
      return `<div class="form-grid cols3">
    <div class="form-group">
      <label>Código de Barras</label>
      <div style="display:flex;gap:4px">
        <input id="pf-cod" value="${p?.codigo || ''}" style="flex:1">
        <button type="button" class="btn btn-sm" onclick="gerarCodigoBarras()" title="Gerar código aleatório" style="padding:0 12px;">Gerar</button>
      </div>
    </div>
    <div class="form-group" style="grid-column:span 2"><label>Nome *</label><input id="pf-nome" value="${p?.nome || ''}" required></div>
    <div class="form-group full"><label>Descrição</label><textarea id="pf-desc">${p?.descricao || ''}</textarea></div>
    <div class="form-group"><label>Categoria</label><select id="pf-catid"><option value="">Sem categoria</option>${catOpts}</select></div>
    <div class="form-group"><label>Unidade</label><select id="pf-un">${['UN', 'KG', 'G', 'L', 'ML', 'MT', 'CM', 'CX', 'PC'].map(u => `<option ${p?.unidade === u ? 'selected' : ''}>${u}</option>`).join('')}</select></div>
    <div class="form-group"><label>Estoque Mínimo</label><input type="number" id="pf-emin" value="${p?.estoque_minimo || 0}" min="0"></div>
    <div class="form-group"><label>Preço Custo (R$)</label><input type="number" id="pf-pcusto" value="${p?.preco_custo || 0}" min="0" step="0.01" onchange="calcMargem()"></div>
    <div class="form-group"><label>Preço Venda (R$)</label><input type="number" id="pf-pvenda" value="${p?.preco_venda || 0}" min="0" step="0.01" onchange="calcMargem()"></div>
    <div class="form-group"><label>Margem</label><input id="pf-margem" readonly style="color:var(--green)"></div>
    <div class="form-group"><label>Estoque Atual</label><input type="number" id="pf-estq" value="${p?.estoque || 0}" min="0"></div>
  </div>`;
    }
    function calcMargem() { const c = parseFloat(document.getElementById('pf-pcusto')?.value || 0) || 0; const v = parseFloat(document.getElementById('pf-pvenda')?.value || 0) || 0; const el = document.getElementById('pf-margem'); if (el) el.value = c > 0 ? ((v - c) / c * 100).toFixed(1) + '%' : '-'; }
    
    function gerarCodigoBarras() {
      const inp = document.getElementById('pf-cod');
      if (!inp) return;
      // Inicia com 789 ou 780 (Brasil)
    ...
    async function novoProduto() { allCategorias = await api('/api/categorias') || []; openModal('Novo Produto', buildProdutoForm(null), `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarProduto()">Salvar</button>`, 'modal-lg'); setTimeout(calcMargem, 50); }
    async function editarProduto(id) { allCategorias = await api('/api/categorias') || []; const p = (await api('/api/produtos'))?.find(x => x.id === id); openModal('Editar Produto', buildProdutoForm(p), `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarProduto(${id})">Salvar</button>`, 'modal-lg'); setTimeout(calcMargem, 50); }

    function getProdutoData() { return { codigo: document.getElementById('pf-cod')?.value || null, nome: document.getElementById('pf-nome')?.value || '', descricao: document.getElementById('pf-desc')?.value || '', categoria_id: document.getElementById('pf-catid')?.value || null, unidade: document.getElementById('pf-un')?.value || 'UN', estoque_minimo: parseFloat(document.getElementById('pf-emin')?.value || 0) || 0, preco_custo: parseFloat(document.getElementById('pf-pcusto')?.value || 0) || 0, preco_venda: parseFloat(document.getElementById('pf-pvenda')?.value || 0) || 0, estoque: parseFloat(document.getElementById('pf-estq')?.value || 0) || 0 }; }

    async function salvarProduto(id = null) { const d = getProdutoData(); if (!d.nome) { notify('Nome é obrigatório', 'error'); return; } const r = id ? await api(`/api/produtos/${id}`, 'PUT', d) : await api('/api/produtos', 'POST', d); if (r?.ok) { notify(id ? 'Produto atualizado!' : 'Produto criado!', 'success'); closeModal(); allProdutos = await api('/api/produtos') || []; loadProdutos(); } else notify('Erro', 'error'); }
    async function deletarProduto(id) { if (!confirm('Inativar produto?')) return; await api(`/api/produtos/${id}`, 'DELETE'); notify('Produto removido', 'info'); allProdutos = await api('/api/produtos') || []; loadProdutos(); }
    ...
    async function renderClientes() {
      ...
      document.getElementById('content').innerHTML = `
    <div class="filters"><div class="filter-group"><label>Buscar</label><input type="text" id="cf-q" placeholder="Nome, CPF/CNPJ, telefone..."></div><div style="display:flex;align-items:flex-end"><button class="btn btn-primary" onclick="loadClientes()">🔍 Buscar</button></div></div>
    <div class="card"><div class="tbl-wrap" id="cli-tbl"><div class="empty"><div class="empty-icon">⏳</div><p>Carregando...</p></div></div></div>`;
      loadClientes();
    }

    async function loadClientes() {
      const q = document.getElementById('cf-q')?.value || ''; const data = await api(`/api/clientes?q=${encodeURIComponent(q)}`); if (!data) return;
      allClientes = data; const tbl = document.getElementById('cli-tbl');
      if (!data.length) { tbl.innerHTML = '<div class="empty"><div class="empty-icon">👤</div><p>Nenhum cliente</p></div>'; return; }
      tbl.innerHTML = `<table><thead><tr><th>Nome</th><th>CPF/CNPJ</th><th>Telefone</th><th>Email</th><th>Endereço</th><th>Cadastro</th><th></th></tr></thead>
  <tbody>${data.map(c => `<tr><td><b>${c.nome}</b></td><td class="mono">${c.cpf_cnpj || '-'}</td><td>${c.telefone || '-'}</td><td>${c.email || '-'}</td><td>${c.endereco || '-'}</td><td>${new Date(c.criado_em).toLocaleDateString('pt-BR')}</td>
  <td style="display:flex;gap:4px"><button class="btn btn-sm" onclick="editarCliente(${c.id})">Editar</button><button class="btn btn-sm btn-danger" onclick="deletarCliente(${c.id})">Del</button></td></tr>`).join('')}</tbody></table>`;
    }

    function buildClienteForm(c) { return `<div class="form-grid"><div class="form-group full"><label>Nome *</label><input id="clf-nome" value="${c?.nome || ''}" required></div><div class="form-group"><label>CPF / CNPJ</label><input id="clf-cpf" value="${c?.cpf_cnpj || ''}"></div><div class="form-group"><label>Telefone</label><input id="clf-tel" value="${c?.telefone || ''}"></div><div class="form-group"><label>Email</label><input type="email" id="clf-email" value="${c?.email || ''}"></div><div class="form-group full"><label>Endereço</label><input id="clf-end" value="${c?.endereco || ''}"></div></div>`; }
    function getClienteData() { return { nome: document.getElementById('clf-nome')?.value || '', cpf_cnpj: document.getElementById('clf-cpf')?.value || '', telefone: document.getElementById('clf-tel')?.value || '', email: document.getElementById('clf-email')?.value || '', endereco: document.getElementById('clf-end')?.value || '' }; }
    async function novoCliente() { openModal('Novo Cliente', buildClienteForm(null), `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarCliente()">Salvar</button>`, 'modal-md'); }
    async function editarCliente(id) { const c = allClientes.find(x => x.id === id); openModal('Editar Cliente', buildClienteForm(c), `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarCliente(${id})">Salvar</button>`, 'modal-md'); }
    async function salvarCliente(id = null) { const d = getClienteData(); if (!d.nome) { notify('Nome é obrigatório', 'error'); return; } const r = id ? await api(`/api/clientes/${id}`, 'PUT', d) : await api('/api/clientes', 'POST', d); if (r?.ok) { notify(id ? 'Cliente atualizado!' : 'Cliente criado!', 'success'); closeModal(); loadClientes(); } else notify('Erro', 'error'); }
    async function deletarCliente(id) { if (!confirm('Excluir cliente?')) return; await api(`/api/clientes/${id}`, 'DELETE'); notify('Excluído', 'info'); loadClientes(); }
    ...
    async function renderFinanceiro() {
      const hasFinanceiro = currentUser.papel === 'admin' || currentUser.permissions.includes('financeiro');
      const hasReceber = currentUser.papel === 'admin' || currentUser.permissions.includes('receber');

      // Default to what the user has access to
      if (!hasFinanceiro && hasReceber) currentFinTab = 'receber';
      else if (hasFinanceiro) currentFinTab = 'fluxo';
    ...
    async function updateFinanceiroView() {
      ...
      // Topbar Actions
      let actionsHtml = '';
      if (currentFinTab === 'fluxo' && hasFinanceiro) {
      ...
      document.getElementById('content').innerHTML = `
        <div class="tabs-wrap">
          ${hasFinanceiro ? `<div class="tab-item ${currentFinTab === 'fluxo' ? 'active' : ''}" onclick="switchFinTab('fluxo')">💸 Fluxo de Caixa</div>` : ''}
    ...
    async function loadFinanceiro() {
      ...
      const dH = desp.length ? desp.map(d => `<tr><td>${d.data}</td><td>${d.descricao}</td><td>${d.categoria}</td><td class="mono text-red fw7">${fmt(d.valor)}</td><td>${d.forma_pagamento}</td><td><button class="btn btn-sm btn-danger" onclick="deletarDespesa(${d.id})">✕</button></td></tr>`).join('') : '<tr><td colspan="6" style="text-align:center;color:var(--text3)">Nenhuma</td></tr>';
      const cH = compr.length ? compr.map(c => `<tr><td>${c.data}</td><td>${c.fornecedor || '-'}</td><td class="mono">${c.numero_nota || '-'}</td><td class="mono text-orange fw7">${fmt(c.total)}</td><td>${c.observacao || '-'}</td></tr>`).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--text3)">Nenhuma</td></tr>';
          ...
          <tr>
            ...
            <td>
              <div style="display:flex;gap:5px;align-items:center;">
                ...
                <button class="icon-btn danger" onclick="excluirConta(${c.id})">🗑️</button>
              </div>
            </td>
          </tr>`;
        });
  ...
  <div class="form-grid">
    ...
    <div class="form-group"><label>Valor *</label><input type="number" id="df-val" min="0.01" step="0.01"></div>
    ...
    async function salvarDespesa() {
      ...
      const d = {
        ...
        valor: parseFloat(document.getElementById('df-val')?.value || 0) || 0,
  ...
  `, `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarCompra()">Confirmar Entrada</button>`, 'modal-lg');
    ...
    function addCompraItem() { const opts = allProdutos.map(p => `<option value="${p.id}" data-nome="${p.nome}">${p.nome}</option>`).join(''); const idx = (window._cpItens || []).length; const el = document.createElement('div'); el.id = `cpi-${idx}`; el.style.cssText = 'display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:8px;margin-bottom:8px;align-items:end'; el.innerHTML = `<select id="cpi-prod-${idx}" onchange="calcCpTotal()"><option value="">Selecione...</option>${opts}</select><input type="number" id="cpi-qtd-${idx}" value="1" min="0.01" step="0.01" placeholder="Qtd" onchange="calcCpTotal()"><input type="number" id="cpi-preco-${idx}" value="0" min="0" step="0.01" placeholder="Preço" onchange="calcCpTotal()"><button class="btn btn-sm btn-danger" onclick="rmCpItem(${idx})">✕</button>`; document.getElementById('cp-itens').appendChild(el); if (!window._cpItens) window._cpItens = []; window._cpItens.push({ idx }); }
    function rmCpItem(idx) { document.getElementById(`cpi-${idx}`)?.remove(); calcCpTotal(); }
    function calcCpTotal() { let t = 0; document.querySelectorAll('[id^="cpi-qtd-"]').forEach(el => { const i = el.id.split('-').pop(); const q = parseFloat(el.value || 0) || 0; const p = parseFloat(document.getElementById(`cpi-preco-${i}`)?.value || 0) || 0; t += q * p; }); const el = document.getElementById('cp-total'); if (el) el.textContent = fmt(t); }
    ...
    async function salvarCompra() {
      ...
      document.querySelectorAll('[id^="cpi-prod-"]').forEach(sel => {
        ...
        const preco = parseFloat(document.getElementById(`cpi-preco-${i}`)?.value || 0) || 0;
    ...
    async function renderRelatorios() {
      if (!allVendedores || !allVendedores.length) allVendedores = await api('/api/vendedores') || [];
      const vendOpts = allVendedores.map(v => `<option value="${v.id}">${v.nome}</option>`).join('');
      document.getElementById('content').innerHTML = `
    <div style="display:flex;gap:10px;margin-bottom:16px">
      <button class="btn btn-primary" onclick="loadRelVendas()">📊 Vendas</button>
      <button class="btn" onclick="loadRelFinanceiro()">💰 Financeiro</button>
      <button class="btn" onclick="loadRelEstoque()">📦 Estoque</button>
    </div>
    <div class="filters" style="display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;margin-bottom:16px">
      ...
      <div class="filter-group" id="filter-vendedor"><label>Vendedor</label><select id="rf-vend" onchange="loadRelVendas()"><option value="">Todos</option>${vendOpts}</select></div>
    </div>
    ...
    async function loadRelVendas() {
      ...
      const di = document.getElementById('rf-di')?.value || firstDay(); const df = document.getElementById('rf-df')?.value || today(); const ag = document.getElementById('rf-agrup')?.value || 'dia';
      const vend = document.getElementById('rf-vend')?.value || '';
      const d = await api(`/api/relatorios/vendas?data_ini=${di}&data_fim=${df}&agrupamento=${ag}&vendedor_id=${vend}`); if (!d) return;
    ...
    async function loadRelFinanceiro() {
      ...
      const di = document.getElementById('rf-di')?.value || firstDay(); const df = document.getElementById('rf-df')?.value || today();
      const d = await api(`/api/relatorios/financeiro?data_ini=${di}&data_fim=${df}`); if (!d) return;
    ...
    async function loadRelEstoque() {
      const g = document.getElementById('filter-vendedor'); if (g) g.style.display = 'none';
      const agr = document.getElementById('rf-agrup'); if (agr) agr.parentElement.style.display = 'none';
      const d = await api('/api/relatorios/estoque'); if (!d) return;
      ...
      document.getElementById('content').innerHTML = `
        <div class="glass-card" style="max-width: 800px; margin: 0 auto;">
          <div style="display:flex; border-bottom:1px solid var(--border); margin-bottom:20px;">
            <div id="tab-geral" class="nav-item active" style="padding:10px 20px; cursor:pointer;" onclick="switchSettingsTab('geral')">Loja</div>
            <div id="tab-vendedores" class="nav-item" style="padding:10px 20px; cursor:pointer;" onclick="switchSettingsTab('vendedores')">Vendedores</div>
            <div id="tab-maquininhas" class="nav-item" style="padding:10px 20px; cursor:pointer;" onclick="switchSettingsTab('maquininhas')">Maquininhas / Taxas</div>
            <div id="tab-usuarios" class="nav-item" style="padding:10px 20px; cursor:pointer;" onclick="switchSettingsTab('usuarios')">Usuários</div>
            <div id="tab-contas" class="nav-item" style="padding:10px 20px; cursor:pointer;" onclick="switchSettingsTab('contas')">&#128181; Contas</div>
          </div>
          ...
          <div id="settings-tab-geral">
            ...
            <div style="display:flex; gap:24px; margin-bottom:30px; align-items:flex-start; flex-wrap:wrap">
              ...
              <div class="form-group">
                ...
                <input type="text" id="cfg-shop-name" value="${shopConfig.shop_name || ''}" placeholder="Ex: Center Cell">
              </div>
              <div class="form-group">
                <label>Endereço Completo</label>
                <input type="text" id="cfg-shop-address" value="${shopConfig.shop_address || ''}" placeholder="Rua, Número, Bairro, Cidade">
              </div>
              <div class="form-group">
                <label>WhatsApp / Telefone</label>
                <input type="text" id="cfg-shop-whatsapp" value="${shopConfig.shop_whatsapp || ''}" placeholder="(11) 99999-9999">
              </div>
              <div class="form-group">
                <label>Instagram</label>
                <input type="text" id="cfg-shop-instagram" value="${shopConfig.shop_instagram || ''}" placeholder="@sua_loja">
              </div>
            </div>
        ...
        <div id="settings-tab-vendedores" style="display:none;">
          ...
          <div id="settings-tab-maquininhas" style="display:none;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
            ...
            <p style="color: var(--text2); font-size: 13px; margin-bottom: 20px;">Cadastre as operadoras que você utiliza para que o sistema calcule o lucro líquido real após as taxas.</p>
            <div id="maquininhas-list">
              <div class="empty"><div class="empty-icon">⏳</div><p>Carregando...</p></div>
            </div>
          </div>

          <div id="settings-tab-usuarios" style="display:none;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
          ...
          <div id="settings-tab-contas" style="display:none;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
              <div class="section-title" style="margin:0;">Contas Financeiras</div>
              <div style="display:flex;gap:8px;">
                <button class="btn" onclick="transferirEntrContas()">&#8652; Transferir</button>
                <button class="btn btn-primary" onclick="novaContaBancaria()">+ Nova Conta</button>
              </div>
            </div>
            <p style="color: var(--text2); font-size: 13px; margin-bottom: 20px;">Gerencie suas contas financeiras. O Caixa recebe automaticamente o valor das vendas e ordens de serviço. Contas bancárias são usadas para despesas, compras e transferências.</p>
    ...
    async function loadUsuariosTab() {
      ...
      if (btn && restam <= 0) {
        btn.disabled = true;
        btn.title = 'Limite de usuários atingido';
      }
      loadUsuarios();
    }
    ...
    async function fazerUploadLogo(input) {
      ...
      try {
        const r = await fetch('/api/config/logo', {
          method: 'POST',
          body: formData
        }).then(res => res.json());
        ...
        if (r.ok) {
          ...
          // Atualiza logo na sidebar imediatamente
          const sidebarLogo = document.getElementById('sidebar-logo');
          if (sidebarLogo) sidebarLogo.src = r.logo_url;
        } else {
          notify(r.message || 'Erro ao enviar logo', 'error');
        }
      } catch (e) {
        notify('Erro na conexão com o servidor', 'error');
      }
    }
    ...
    async function loadContasView() {
      ...
      if (!contas.length) {
        container.innerHTML = '<div class="empty"><div class="empty-icon">🏦</div><p>Nenhuma conta cadastrada</p></div>';
        return;
      }
      container.innerHTML = `
            ...
            <div class="card" style="padding:20px;position:relative">
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
                ...
                <div>
                  ...
                  <div style="font-size:11px;color:var(--text3)">${isCaixa ? 'Conta Principal (Caixa)' : 'Conta Bancária'}</div>
                </div>
              </div>
              <div style="font-size:22px;font-weight:800;color:${saldoColor};font-family:monospace;margin-bottom:14px">${fmt(c.saldo)}</div>
              <div style="display:flex;gap:6px">
                ${!isCaixa ? `<button class="btn btn-sm" onclick="editarConta(${c.id},'${c.nome.replace(/'/g, "\\'")}')">✏️ Editar</button>` : ''}
                ${!isCaixa ? `<button class="btn btn-sm btn-danger" onclick="excluirConta(${c.id})">🗑️</button>` : ''}
              </div>
            </div>`;
          }).join('')}
        </div>
    ...
    async function novaContaBancaria() {
      openModal('Nova Conta Bancária', `
        <div class="form-grid">
          ...
          <div class="form-group full"><label>Saldo Inicial (R$)</label><input type="number" id="nc-saldo" value="0" min="0" step="0.01" placeholder="0.00"></div>
        </div>
      `, `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarNovaConta()">Criar Conta</button>`, 'modal-sm');
    }

    async function salvarNovaConta() {
      const nome = document.getElementById('nc-nome')?.value?.trim();
      const saldo_inicial = parseFloat(document.getElementById('nc-saldo')?.value || 0) || 0;
    ...
    async function editarConta(id, nomeAtual) {
      openModal('Editar Conta', `
        <div class="form-group"><label>Nome da Conta *</label><input id="ec-nome" value="${nomeAtual}"></div>
      `, `<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarEditarConta(${id})">Salvar</button>`, 'modal-sm');
    }

    async function salvarEditarConta(id) {
    ...
    async function transferirEntrContas() {
      const contas = await api('/api/contas') || [];
      ...
      openModal('Transferência entre Contas', `
        <div class="form-grid">
          ...
          <div class="form-group"><label>Valor *</label><input type="number" id="tr-val" min="0.01" step="0.01" placeholder="0.00"></div>
    ...
    async function confirmarTransferencia() {
      ...
      const valor = parseFloat(document.getElementById('tr-val')?.value || 0) || 0;
    ...
    async function loadMaquininhasView() {
      ...
      if (!data.length) {
        container.innerHTML = '<div class="empty"><div class="empty-icon">💳</div><p>Nenhuma maquininha cadastrada</p></div>';
        return;
      }
      container.innerHTML = `
        <div class="card">
            ...
            <table>
              ...
              <tbody>
                ${data.map(m => `
                  <tr>
                    <td><b>${m.nome}</b></td>
                    <td class="mono">${m.taxa_debito}%</td>
                    <td class="mono">${m.taxa_credito_1x || 0}%</td>
                    <td class="mono">${m.taxa_credito_2x || 0}%</td>
                    <td class="mono">${m.taxa_credito_3x || 0}%</td>
                    <td style="display:flex;gap:4px">
                      <button class="btn btn-sm" onclick="editarMaquininha(${m.id})">Editar</button>
                      <button class="btn btn-sm btn-danger" onclick="deletarMaquininha(${m.id}, '${m.nome}')">Excluir</button>
                    </td>
                  </tr>
                `).join('')}
              </tbody>
    ...
    async function salvarVendedor(id = null) {
      ...
      if (r?.ok) {
        notify(id ? 'Vendedor atualizado!' : 'Vendedor criado!', 'success');
        closeModal();
        loadVendedoresView();
      } else {
    ...
    async function deletarVendedor(id, nome) {
      if (!confirm(`Remover vendedor "${nome}"? (Ele não será apagado das vendas passadas)`)) return;
    ...
    async function salvarConfig() {
      ...
      if (r?.ok) {
        ...
        await initShopConfig(); // Recarregar globalmente
      } else {
